document.addEventListener("DOMContentLoaded", () => {
  ...
  function onNext(evt) {
    ...
    switch (nxt.id) {
      ...
      case "scene-13": setupScene13();  break;
    }
  }
...
function animateIPInv(inputGrid, outputGrid, wrap, onComplete) {
 ...
 // предполагается, что глобально есть:
    // generatedKeys (массив из 16 строк по 48 бит),
    // IP_EXPAND (массив позиций),
    // SBoxes (массив 8 S-блоков),
    // P (массив позиций).
    let roundIndex12 = 1;
    let stepIndex12  = 0;
    let lArr12 = [], rArr12 = [];
    ...
    function flyBit12(srcCell, dstCell, onDone) {
        const wrap = document.getElementById("scene12Wrap");
        const wrapR = wrap.getBoundingClientRect();
        const sR    = srcCell.getBoundingClientRect();
        const dR    = dstCell.getBoundingClientRect();
        const flyer = srcCell.cloneNode(true);
  ...
  X48.innerHTML = "";
  const keyBits = reversedKeys[roundIndex12 - 1]
                    .split("")
                    .map(b => +b);
  const X48arr = Earr.map((b,i)=> b ^ keyBits[i]);
  X48arr.forEach((res,i) => {
  ...
  // 3) S-блоки → 32 бита
  const sContainer = document.getElementById("scene12SBoxes");
  sContainer.innerHTML = "";
  let Sout = [];
  for (let s = 0; s < 8; s++) {
    const chunk = X48arr.slice(s*6, s*6+6);
  ...
  if (roundIndex12 < 16) {
    ...
    document.getElementById("scene12ToFinalBtn")
            .classList.remove("hidden");
  }
    }

    function setupScene12() {
    // 1) Снимаем финальный шифртекст
const cipherBits = Array.from(
    document.getElementById("scene10Output").children
  ).map(c => c.textContent);
  
  // 2) Применяем **начальную** перестановку IP
  const ipBits = IP.map(pos => cipherBits[pos - 1]);
  
  // 3) Делим на L₀ и R₀
  lArr12 = ipBits.slice(0, 32);
  rArr12 = ipBits.slice(32)

      // 3) рисуем ключи в обратном порядке
      reversedKeys = generatedKeys.slice().reverse();
      ...
      // 5) чистим контейнеры и рисуем R0
      ...
      // 6) кнопки
      document.getElementById("scene12NextBtn").textContent = "Шаг 1: Expand";
      ["scene12SkipBtn","scene12NextBtn"].forEach(id=>{
        const b=document.getElementById(id);
        b.classList.remove("hidden"); b.disabled = false;
      });
      ["scene12NextRoundBtn","scene12ToFinalBtn"].forEach(id=>
        document.getElementById(id).classList.add("hidden")
      );

      document.getElementById("scene12NextBtn")
        .onclick = nextSubStep12;
        const skip = document.getElementById("scene12SkipBtn");
        skip.onclick = skipAllSteps12;
        
    }

    function nextSubStep12() {
      switch (stepIndex12) {
        case 0: doExpand_12();  break;
        case 1: doXor48_12();  break;
        case 2: doSBoxes_12();  break;
        case 3: doPPerm_12();   break;
        case 4: doXor32_12();  break;
        case 5: doSwap_12();    break;
      }
      stepIndex12++;

      if (stepIndex12 < 6) {
        // обновляем текст и подсветку
        const labels = ["Expand","XOR₄₈","S-блоки","P-perm","XOR₂","Swap"];
        setStepBar12(stepIndex12+1);
        document.getElementById("scene12NextBtn")
                .textContent = `Шаг ${stepIndex12+1}: ${labels[stepIndex12]}`;
      } else {
        // конец раунда — полностью скрываем кнопку шага и «пропустить»
        document.getElementById("scene12NextBtn").classList.add("hidden");
        document.getElementById("scene12SkipBtn").classList.add("hidden");
...
function doExpand_12() {
    ...
    IP_EXPAND.forEach((rPos, i) => {
      const src = document.getElementById("scene12R").children[rPos-1];
      const dst = Ebox.children[i];
      const bit = rArr12[rPos-1];
      scheduleNext12(() => {
        src.classList.add('glow');
        setTimeout(()=>src.classList.remove('glow'),600);
        flyBit12(src, dst, () => {
  ...
  function doSBoxes_12() {
    ...
    for (let s=0; s<8; s++) {
      ...
      box.innerHTML = `<div class="title">S${s+1}</div>`;
      const grid = document.createElement("div");
      grid.className = "bit-grid scene12-32";
      grid.style.gridTemplateColumns = "repeat(4, var(--bit-size))";
      box.appendChild(grid);
      container.appendChild(box);
      scheduleNext12(() => {
        ...
        bits.forEach((b,j) => {
          ...
          dst.className = "bit-cell bit-green";
          grid.appendChild(dst);
          flyBit12(xorCells[s*6+j], dst, ()=>dst.textContent=b);
        });
      }, delay);
      delay += 500;
    }
  ...
  function doPPerm_12() {
    ...
    const outCells = Array.from(
      document.querySelectorAll("#scene12SBoxes .bit-cell.bit-green")
    );
    const Pbox = document.getElementById("scene12P");
    Pbox.innerHTML = "";
    P.forEach((pos,i) => {
  ...
  function doSwap_12() {
    ...
    prevR.forEach((src,i) => {
      ...
      scheduleNext12(() => {
        src.classList.add('glow');
        setTimeout(()=>src.classList.remove('glow'),600);
        flyBit12(src, dst, () => dst.textContent = src.textContent);
      }, i*40);
    });
    ...
    // Здесь нужно реализовать функции doExpand12, doXor48_12, doSBoxes12, doPPerm12,
    // doXor32_12, doSwap12 аналогично тому, как это сделано в сцене 9,
    // но заменяя generatedKeys[...] на reversedKeys[...] и id на scene12…

    document.getElementById("scene-12")
      .addEventListener("scene:show", setupScene12);


      function setupScene13() {
        // 1) Собираем финальные биты: сначала R₁₆, затем L₁₆
        const finalBits = rArr12.concat(lArr12);
      
        // 2) Применяем финальную перестановку IP⁻¹
        const permuted = IP_INV.map(pos => finalBits[pos - 1]);
      
        // 3) Рисуем сетку бит
        const bitsContainer = document.getElementById("scene13Bits");
        ...
        // 4) Собираем читаемый текст из байт (8 бит → символ)
        const bytes = permuted.join("").match(/.{1,8}/g) || [];
        const chars = bytes.map(b => String.fromCharCode(parseInt(b, 2)));
        document.getElementById("scene13Text").textContent = chars.join("");
      
        // 5) Кнопка «Начать сначала»
        document.getElementById("scene13RestartBtn").onclick = () => {
          window.location.reload();
        };
      }
      
      document.getElementById("scene-13")
              .addEventListener("scene:show", setupScene13);
